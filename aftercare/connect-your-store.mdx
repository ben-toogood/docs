...
7. In the replace box, copy and paste `https://tracking.tryhutch.co.uk/demo?order={{ name | encode_url_component }}&postcode={{ shipping_address.zip }}`. The red highlighted area needs to be replaced with your brand/account name. For example if your brand name was Pimento Fulfulfilment then demo would be replaced by hutch-fulfilment or if the brand name was just Pimento, demo would be replaced with hutch. `https://tracking.tryhutch.co.uk/hutch?order={{ name | encode_url_component }}&postcode={{ shipping_address.zip }}`. Hit “replace all”.
